...
import csv from '../../utils/analytics-Sdatabase.csv'
// import csv from '../../utils/test-data.csv'
import map from '../../utils/us-states.json'
import { Box, Grid, Button } from '@mui/material'
import { mapChartFunc, timeSeriesChartFunc, brandBarChartFunc, retailerBarChartFunc, categoryBarChartFunc, itemBarChartFunc, itemMarBarChartFunc, agePieChartFunc, cityPieChartFunc, dataTableFunc } from '../../components/CrossfilterCharts'
...
const ChartGrid = styled(Box)`
    ...
    width: 100%;
    height: ${(props) => props.bar ? '350px' : '350px'};
`
...
const AnalyticsSales = (props) => {
    ...
    const itemMarBarChartRef = useRef(null)
    ...
    const [itemMarBarChartContainerSize, setItemMarBarChartContainerSize] = useState({ width: 0, height: 0 })
    ...
    useEffect(() => {
        (async () => {
            ...
            data.forEach((d) => {
                ...
                d.Margins = +(d.Margin.slice(1).replace(/\,/g, ''))
    ...
    const setParentSizes = () => {
        ...
        setItemMarBarChartContainerSize(getParentSize(itemMarBarChartRef))
    ...
    useEffect(() => {
        if (cx) {
            ...
            const itemMarBarChart = itemMarBarChartFunc(itemMarBarChartRef.current, cx, minSales, maxSales)
            ...
            const handleResize = debounce(() => {
                ...
                */
                ...
                itemMarBarChart.width(itemMarBarChartContainerSize.width).height(250)

                // countyPieChart.width(countyPieChartContainerSize.width * 0.9).height(300)
                agePieChart.width(agePieChartContainerSize.width * 0.9).height(250)
    ...
    }, [props.renderDashboard, cx, mapChartContainerSize.width, timeSeriesContainerSize.width, brandBarChartContainerSize.width, retailerBarChartContainerSize.width, categoryBarChartContainerSize.width, itemBarChartContainerSize.width, itemMarBarChartContainerSize.width, agePieChartContainerSize.width, cityPieChartContainerSize.width])
        ...
        <AnalyticsDashboardGrid container spacing={2}>
            ...
            <Grid item xs={12} lg={6}>
                <ChartGrid pie id="agePieChart" ref={agePieChartRef}>
                    <ChartHeader>
                        <ChartTitle className="chart-title">Store Wise |</ChartTitle>
                        <Button sx={{ textTransform: 'none' }} onClick={handleResetClick}>reset</Button>
                    </ChartHeader>
                </ChartGrid>
            </Grid>
            <Grid item xs={12} sm={12} lg={6}>
                <ChartGrid bar id="categoryBarChart" ref={categoryBarChartRef}>
                    <ChartHeader>
                        <ChartTitle className="chart-title">Top 10 Categories Sales Wise |</ChartTitle>
                        <Button sx={{ textTransform: 'none' }} onClick={handleResetClick}>reset</Button>
                    </ChartHeader>
                </ChartGrid>
            </Grid>
            <Grid item xs={12} sm={12} lg={6}>
                <ChartGrid bar id="itemBarChart" ref={itemBarChartRef}>
                    <ChartTitle className="chart-title">Top 10 Items  Sales Wise</ChartTitle>
                </ChartGrid>
            </Grid>
            <Grid item xs={12} sm={12} lg={6}>
                <ChartGrid bar id="itemBarChart" ref={itemMarBarChartRef}>
                    <ChartTitle className="chart-title">Top 10 Items Margin Wise</ChartTitle>
                </ChartGrid>
            </Grid>
